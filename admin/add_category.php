...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $conn->close();
    header("Location: add_category.php"); // Redirect back to category page after adding
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    header("Location: add_category.php"); // Redirect back to the page
    exit();
}
    ...
    <li class="mb-3"><a href="dashboard.php" class="flex items-center hover:text-gray-300"><i class="fas fa-tachometer-alt mr-2"></i> Dashboard</a></li>
     ...
     <div class="flex-grow p-6 bg-gray-100">
        ...
        <div class="bg-white p-8 rounded shadow-md mb-5">
            <h2 class="text-2xl font-bold mb-4 text-center">Add New Category</h2>
            <form id="categoryForm" method="POST" action="add_category.php">
                <div class="mb-4">
                    <label for="category_name" class="block text-sm font-medium text-gray-700">Category Name</label>
                    <input type="text" name="category_name" id="category_name" required class="mt-1 p-2 border border-gray-300 rounded w-full" />
                </div>
                <div class="flex justify-center">
                    <button type="button" id="addCategoryButton" class="bg-blue-500 text-white font-bold py-2 px-4 rounded">Add Category</button>
                </div>
            </form>
        </div>
            ...
            <table class="min-w-full border-collapse">
                ...
                <tbody>
                    <?php while ($row = $categories->fetch_assoc()) : ?>
                        <tr>
                            <td class="border p-2"><?php echo htmlspecialchars($row['category_name']); ?></td>
                            <td class="border p-2">
            ...
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = `add_category.php?delete=${categoryId}`;
                    }
                });
            });
        });
